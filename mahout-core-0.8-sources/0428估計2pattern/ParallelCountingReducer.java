...
public class ParallelCountingReducer extends Reducer<Text,Text,Text,LongWritable> {
  ...
  public static long T_number = 88162;
...
//================================================================================================================
	...
	if(sum>=Bound){	//B
		...
		for(int i=0;i<B_list.size();i++){
			if(sum<B_list.get(i))
						outFile.writeBytes(String.valueOf(++two_pattern_count)+".["+B_ID.get(i)+", "+T_ID+"]\n");
			else
				outFile.writeBytes(String.valueOf(++two_pattern_count)+".["+T_ID+", "+B_ID.get(i)+"]\n");
		}
		...
		if(relation_rate>0){	//real > ess
			if(Ess>= minSupport){
				outFile.writeBytes(String.valueOf(++two_pattern_count)+".["+T_ID+", "+S_ID.get(i)+"]\n");
				}
		...
		for(int i=0;i<S_list.size();i++){	//for SS
			...
			if(relation_rate>0){
				if(Ess+relation_rate*T_number>minSupport){
					if(sum<S_list.get(i))
						outFile.writeBytes(String.valueOf(++two_pattern_count)+".["+S_ID.get(i)+", "+T_ID+"]\n");
					else
						outFile.writeBytes(String.valueOf(++two_pattern_count)+".["+T_ID+", "+S_ID.get(i)+"]\n");
				}
			}
