...
public class ParallelCountingReducer extends Reducer<Text,Text,Text,LongWritable> {
  ...
  public static long number_of_group = 9000;
...
//			tmp += value.toString().split(",")[1]+"#";
	...
	double LMS = group_size*(minSupport/number_of_group);
	...
	if(sum>=Bound){	//B
		...
		for(int i=0;i<S_list.size();i++){	//for BS
			if(S_list.get(i)*sum >= T_number*minSupport){
				outFile.writeBytes(String.valueOf(++two_pattern_count)+"\n");
				}
			else{
					long Expectations_success = 0;	
					for(int j=0;j<number_of_group;j++){
						if(group[j]*(S_table_list.get(i)[j])/group_size >= (minSupport/number_of_group)-S(group[j],S_table_list.get(i)[j],group_size))
							Expectations_success ++;
						}
					if(Expectations_success >= number_of_group*0.68){
					outFile.writeBytes(String.valueOf(++two_pattern_count)+"\n");
					}
			}
		}
	...
	if(sum >= minSupport && sum < Bound){	//S
		...
		for(int i=0;i<B_list.size();i++){	//for SB
			if(B_list.get(i)*sum >= T_number*minSupport){
				outFile.writeBytes(String.valueOf(++two_pattern_count)+"\n");
				}
			else{
					long Expectations_success = 0;	
					for(int j=0;j<number_of_group;j++){
						if(group[j]*(B_table_list.get(i)[j]) >= (minSupport/number_of_group)-S(group[j],S_table_list.get(i)[j],group_size))
							Expectations_success ++;
						}
					if(Expectations_success >= number_of_group*0.68){
					outFile.writeBytes(String.valueOf(++two_pattern_count)+"\n");
...
/*	
  ...
  protected void setup(Context context) throws IOException, InterruptedException {
	...
	public static double S(long a,long b,long X){
		double M = a*b/X;
		return Math.sqrt(M*((a-1)*(b-1)/(X-1))+M-M*M);
	}
