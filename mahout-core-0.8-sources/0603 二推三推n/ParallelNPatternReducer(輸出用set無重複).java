...
public class ParallelNPatternReducer extends Reducer<Text,Text,Text,Text> {
  ...
  protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException,
	...
	Boolean_sample_list.add(SampleListToBoolean(str_value[2]));
	 ...
	 HashSet<HashSet<Integer>> ID_set= new  HashSet<HashSet<Integer>>();
	...
	for(int i=0;i<item.size();i++){//2 to 3
		for(int j=0;j<i;j++){
			...
			if(Ess.get(i)*Ess.get(j)*(1/(double)Keycount)+relation_rate*T_number >= minSupport){
				 ...
				 HashSet<Integer> set = new HashSet<Integer>(IDList);
...
//===========================================================================================================================
if(ID_set.size()>0)
			outFile.writeBytes(Keyitem+":("+String.valueOf(ID_set.size())+")"+String.valueOf(ID_set)+"\n");

	HashSet<HashSet<Integer>> new_ID_set = new HashSet<HashSet<Integer>>();
	...
	if(ID.size()<2 || (debug++)==10000)break;
	//if(ID.size()>1){
	...
	for(int i=0;i<ID.size();i++){
		...
		for(int j=0;j<i;j++){
			...
			HashSet<Integer> tmp_ID_set= new HashSet<Integer>(tmp_ID);
	...
	//else outFile.writeBytes(Keyitem+": null"+"\n");
