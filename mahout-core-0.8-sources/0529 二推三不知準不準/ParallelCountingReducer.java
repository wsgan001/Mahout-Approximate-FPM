	...
	if(sum>=Bound){	//B
		...
		String second_sample = String.valueOf(fp_list.get(fp_size-1));//self
			if(sum<B_list.get(i)){
				...
				outFile.writeBytes(B_ID.get(i)+":"+B_list.get(i)+":"+first_sample+"#"+T_ID+":"+sum+":"+second_sample+"_"+Ess+"#"+String.valueOf(fp_list)+"\n");
				}
			else{
				//outFile.writeBytes(String.valueOf(++two_pattern_count)+".["+T_ID+", "+B_ID.get(i)+"] "+String.valueOf(relation_rate)+"\n");
				outFile.writeBytes(T_ID+":"+sum+":"+second_sample+"#"+B_ID.get(i)+":"+B_list.get(i)+":"+first_sample+"_"+Ess+"#"+String.valueOf(fp_list)+"\n");
				}
		}
		...
		if(relation_rate>0){	//real > ess
			if(Ess>= minSupport){
				outFile.writeBytes(T_ID+":"+sum+":"+second_sample+"#"+S_ID.get(i)+":"+S_list.get(i)+":"+first_sample+"_"+Ess+"#"+String.valueOf(fp_list)+"\n");
				}
		...
		for(int i=0;i<S_list.size();i++){	//for SS
			...
			if(relation_rate>0){
				if(Ess+relation_rate*T_number>minSupport){
					if(sum<S_list.get(i))
						outFile.writeBytes(S_ID.get(i)+":"+S_list.get(i)+":"+first_sample+"#"+T_ID+":"+sum+":"+second_sample+"_"+Ess+"#"+String.valueOf(fp_list)+"\n");
					else
						outFile.writeBytes(T_ID+":"+sum+":"+second_sample+"#"+S_ID.get(i)+":"+S_list.get(i)+":"+first_sample+"_"+Ess+"#"+String.valueOf(fp_list)+"\n");
				}
			}
