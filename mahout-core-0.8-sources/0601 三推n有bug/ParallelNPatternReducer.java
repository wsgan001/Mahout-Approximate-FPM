...
//===========================================================================================================================
	...
	for(int i=0;i<ID.size();i++){
		...
		for(int j=0;j<i;j++){
			...
			for(int k=0;k<tmp_C.size();k++)multi_cl*=tmp_C.get(k);
			for(int k=0;k<tmp_SC.size();k++)multi_scl*=tmp_SC.get(k);
			for(int k=0;k<status+1;k++){ts*=T_number;ss*=sample_number;}
			multi_cl *= Keycount;
			multi_scl *= KeySampleCount;
			
			double essti = (double)multi_cl/(double)ts;
			double rr = ((double)(tmp_FP.size())-(double)multi_scl/(double)ss)/sample_number;
			if(essti+rr*T_number >= minSupport){
				new_ID.add(tmp_ID);
				new_C.add(tmp_C);
				new_SC.add(tmp_SC);
				new_FP.add(tmp_FP);
				//outFile.writeBytes(status+":"+Keyitem+" ");
