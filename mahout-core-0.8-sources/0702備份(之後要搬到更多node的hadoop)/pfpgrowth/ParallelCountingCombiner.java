...
private int minSupport = 3;
  ...
  public static OpenObjectIntHashMap<Integer> SampleTransection = PFPGrowth.SampleTransection;
  @Override
...
//	Matcher isNum = pattern.matcher(value.toString().split(",")[0]);
	  ...
	  int ConvertTID = Integer.parseInt(value.toString().split("T")[1]);
	  if(SampleTransection.containsKey(ConvertTID))
		tmp.append("T"+Integer.toString(SampleTransection.get(ConvertTID)));	//add T_ID
...
//		group[Integer.valueOf(value.toString().split("T")[1])%number_of_group]++;
	  ...
	  /*
		if(str_value[0].split("#").length>1){//i#g[i]
			int G_key = Integer.valueOf(str_value[0].split("#")[0]);
			int G_value = Integer.valueOf(str_value[0].split("#")[1]);
			if(map.get(G_key)==null)map.put(G_key,G_value);
			else map.put(G_key,map.get(G_key)+G_value);
		}*/
...
//			tmp.append(value.toString().split(",")[1]+"#");
		...
		for(int i=1;i<str_value.length;i++){
			sample.add(Integer.valueOf(str_value[i]));
			//int G_ID = Integer.valueOf(str_value[i])%number_of_group;
			//if(map.get(G_ID)==null)map.put(G_ID,1);
			//else map.put(G_ID,map.get(G_ID)+1);
		}
...
//		}
		//if(c!=0)System.out.println(key.toString()+","+String.valueOf(c)+","+String.valueOf(d));
	if(a!=0){//to combine
...
//		output_value = String.valueOf(sum)+",";
		if(sum > minSupport*0.025){//Threshold
			//for (Object g_key : map.keySet()) {
			//		context.write(key,new Text(String.valueOf(g_key)+"#"+String.valueOf(map.get(g_key))));
			//	}
			for(int i=0;i<sample.size();i++){
				context.write(key,new Text("#"+String.valueOf(sample.get(i))));
			}
			}
