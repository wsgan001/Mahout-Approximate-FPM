...
public static final int HowManyStd=PFPGrowth.HowManyStd;
	...
	String [] str_value = value.toString().split("#");
      context.setStatus("Parallel Counting Reducer :" + key);
		if(str_value.length == 1){	 
			...
			T_sample[Integer.parseInt(str_value[1])]=true;
		}
...
//================================================================================================================
	...
	if(sum>=Bound){	//B
		...
		String second_sample = String.valueOf(fp_list.get(fp_size-1));//self
				...
				if(WhichSmallString(B_ID.get(i),T_ID))context.write(new Text(B_ID.get(i)+"#"+T_ID), new LongWritable(1));
				else context.write(new Text(T_ID+"#"+B_ID.get(i)), new LongWritable(1));
		...
		if(relation_rate>0){	//real > ess
			if(Ess>= minSupport ||(Ess<minSupport&&Ess+relation_rate*T_number>minSupport)){
				...
				if(WhichSmallString(S_ID.get(i),T_ID))context.write(new Text(S_ID.get(i)+"#"+T_ID), new LongWritable(1));
				else context.write(new Text(T_ID+"#"+S_ID.get(i)), new LongWritable(1));
				}
		}
  ...
  protected void setup(Context context) throws IOException, InterruptedException {
	...
	public static double V(long a,long b,long X){
	 ...
	 public static double relation(boolean [] first,boolean [] second,long X){
		 ...
		 double ess = E(a,b,X);
		 double std = S(a,b,X);
		 double uper_bound = ess+HowManyStd*std;
		 double lower_bound = ess-HowManyStd*std;
