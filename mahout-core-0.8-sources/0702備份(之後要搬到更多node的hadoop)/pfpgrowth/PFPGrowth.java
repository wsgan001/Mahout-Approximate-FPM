...
public final class PFPGrowth {
  ...
  public static final long T_number = 8200000;
  public static final int sample_number = 10000;  
  public static final int HowManyStd=7;
  public static final OpenObjectIntHashMap<Integer> SampleTransection = GetSample() ;
   ...
   */
   public static OpenObjectIntHashMap<Integer> GetSample(){
		OpenObjectIntHashMap<Integer> tmp = new OpenObjectIntHashMap<Integer>();
		Random ran = new Random();
		while(true){
        	if(sample_count==sample_number)break;
			int Snum = ran.nextInt((int)T_number);
			if(tmp.containsKey(Snum))continue;
        	tmp.put(Snum,sample_count++);
        }
		return tmp;
   }
 ...
 * @params
  ...
  public static void runPFPGrowth(Parameters params, Configuration conf) throws IOException,
	...
	for(int i=0;i<2;i++){
		params.set("status", Integer.toString(status));
		startNPattern(params, conf,status++);
	}
	...
	System.out.println("WordCount Times(Milliseconds): "+String.valueOf((end-begin)*0.001)+" seconds");
	System.out.println("mahout run time: "+String.valueOf((end2-begin)*0.001)+" seconds");
  }
  ...
  public static void startParallelCounting(Parameters params, Configuration conf)
    ...
    job.setCombinerClass(ParallelCountingCombiner.class);
  ...
  //=============================================================================================================
  public static void startNPattern(Parameters params, Configuration conf,int stat)
    throws IOException, InterruptedException, ClassNotFoundException {
    conf.set(PFP_PARAMETERS, params.toString());
    ...
    String input = "/user/root/output/result_"+Integer.toString(stat-1);
    Job job = new Job(conf, "Parallel Counting Driver running over input: " + input);
    job.setJarByClass(PFPGrowth.class);
    ...
    FileInputFormat.addInputPath(job, new Path(input));
    Path outPath = new Path("/user/root/output/result_N");
    FileOutputFormat.setOutputPath(job, outPath);
    
    HadoopUtil.delete(conf, outPath);
    
    job.setInputFormatClass(TextInputFormat.class);
    job.setMapperClass(ParallelNPatternMapper.class);
    //job.setCombinerClass(ParallelNPatternReducer.class);
    job.setReducerClass(ParallelNPatternReducer.class);
    job.setOutputFormatClass(SequenceFileOutputFormat.class);
