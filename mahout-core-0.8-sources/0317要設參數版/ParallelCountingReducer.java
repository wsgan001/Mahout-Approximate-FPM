			...
			if(Ess>= minSupport){
				outFile.writeBytes(String.valueOf(++two_pattern_count)+"\n");
					...
					for(int j=0;j<number_of_group;j++){
						...
						if(E(group[j],S_table_list.get(i)[j],group_size)+S(group[j],S_table_list.get(i)[j],group_size) >= LMS)
							Expectations_success ++;
						}
					if(Expectations_success >= number_of_group*0.1){
					outFile.writeBytes(String.valueOf(++two_pattern_count)+"\n");
					}
			}
			...
			if(Ess>= minSupport){
				outFile.writeBytes(String.valueOf(++two_pattern_count)+"\n");
					...
					for(int j=0;j<number_of_group;j++){
						...
						if(E(group[j],B_table_list.get(i)[j],group_size)+S(group[j],B_table_list.get(i)[j],group_size) >= LMS)
							Expectations_success ++;
						}
					if(Expectations_success >= number_of_group*0.1){
					outFile.writeBytes(String.valueOf(++two_pattern_count)+"\n");
