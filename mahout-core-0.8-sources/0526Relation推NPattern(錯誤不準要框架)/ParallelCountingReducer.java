	...
	if(sum>=Bound){	//B
		...
		double relation_rate = relation(B_sample_list.get(i),T_sample,sample_number);
			if(sum<B_list.get(i)){
				...
				outFile.writeBytes(B_ID.get(i)+":"+B_list.get(i)+"#"+T_ID+":"+sum+" "+Ess+","+String.valueOf(relation_rate)+"\n");
				}
			else{
				//outFile.writeBytes(String.valueOf(++two_pattern_count)+".["+T_ID+", "+B_ID.get(i)+"] "+String.valueOf(relation_rate)+"\n");
				outFile.writeBytes(T_ID+":"+sum+"#"+B_ID.get(i)+":"+B_list.get(i)+" "+Ess+","+String.valueOf(relation_rate)+"\n");
				}
		}
		...
		if(relation_rate>0){	//real > ess
			if(Ess>= minSupport){
				outFile.writeBytes(T_ID+":"+sum+"#"+S_ID.get(i)+":"+S_list.get(i)+" "+Ess+","+String.valueOf(relation_rate)+"\n");
				}
		...
		for(int i=0;i<S_list.size();i++){	//for SS
			...
			if(relation_rate>0){
				if(Ess+relation_rate*T_number>minSupport){
					if(sum<S_list.get(i))
						outFile.writeBytes(S_ID.get(i)+":"+S_list.get(i)+"#"+T_ID+":"+sum+" "+Ess+","+String.valueOf(relation_rate)+"\n");
					else
						outFile.writeBytes(T_ID+":"+sum+"#"+S_ID.get(i)+":"+S_list.get(i)+" "+Ess+","+String.valueOf(relation_rate)+"\n");
				}
			}
